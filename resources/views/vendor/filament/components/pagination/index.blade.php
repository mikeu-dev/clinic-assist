        ...
        <span class="fi-pagination-overview">
            {{
                trans_choice(
                    ...
                    $paginator->total(),
                    [
                        'first' => extension_loaded('intl') ? Number::format($paginator->firstItem() ?? 0) : number_format($paginator->firstItem() ?? 0),
                        'last' => extension_loaded('intl') ? Number::format($paginator->lastItem() ?? 0) : number_format($paginator->lastItem() ?? 0),
                        'total' => extension_loaded('intl') ? Number::format($paginator->total()) : number_format($paginator->total()),
                    ],
                )
            }}
        </span>
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        <x-filament::pagination.item
                            ...
                            :aria-label="trans_choice('filament::components/pagination.actions.go_to_page.label', $page, ['page' => extension_loaded('intl') ? Number::format($page) : $page])"
                            :label="extension_loaded('intl') ? Number::format($page) : (string)$page"
